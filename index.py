...
buyinguser = []
bankchanginguser = []
total_master_ids = [828472376030265397] #총관리자
...
async def on_ready():
    ...
    while True:
        await bot.change_presence(activity=discord.Game(f"ButtonService | {len(bot.guilds)}서버 사용"),status=discord.Status.online)
...
async def on_message(message):
                ...
                if 1 <= amount <= 30:
                    try:
                        ...
                        if 30 <= license_length <= 9999:
                            ...
                            eb = DiscordEmbed(title='라이센스 생성 로그', description=f'```유저 : {message.author.name}#{message.author.discriminator} ({message.author.id})\n개수 : {message.content.split(" ")[1]}\n기간 : {message.content.split(" ")[2]} 일\n라이센스 : {"\n".join(codes)}```', color=0x5c6cdf)
                            webhook.add_embed(eb)
                            webhook.execute()
                        else:
                            await message.channel.send(embed=discord.Embed(title="생성 실패", description="최소 30일부터 최대 9999일 사이로만 생성 가능합니다.", color=0xff0000))
                else:
                    await message.channel.send(embed=discord.Embed(title="생성 실패", description="최대 30개까지 생성 가능합니다.", color=0xff0000))
    ...
    if message.content == "!set":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                cur = con.cursor()
                cur.execute("ALTER TABLE serverinfo ADD bankm INTEGER;")
                con.commit()
                cur.execute("ALTER TABLE serverinfo ADD mm INTEGER;")
                con.commit()
                cur.execute("ALTER TABLE serverinfo ADD totalm INTEGER;")
                con.commit()
                con.close
                await message.channel.send(embed=discord.Embed(title="세팅 성공", description="데이터베이스 컬럼 추가가 완료되었습니다.\n**!setup**을 입력해주세요.", color=0x5c6cdf))
    ...
    if message.content == "!setup":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                con.commit()
                con.close
                await message.channel.send(embed=discord.Embed(title="세팅 성공", description="데이터베이스 컬럼이 성공적으로 수정되었습니다.", color=0x5c6cdf))
    ...
    if message.content == "!초기화 문상":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                con.commit()
                con.close
                await message.channel.send(embed=discord.Embed(title="초기화 성공", description="문화상품권 누적 충전금액이 0원으로 수정되었습니다.", color=0x5c6cdf))
    ...
    if message.content == "!초기화 계좌":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                con.commit()
                con.close
                await message.channel.send(embed=discord.Embed(title="초기화 성공", description="계좌이체 누적 충전금액이 0원으로 수정되었습니다.", color=0x5c6cdf))

    if message.content == "!초기화 토탈":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                await message.delete()
                con = sqlite3.connect("../DB/" + str(message.guild.id) + ".db")
                ...
                con.commit()
                con.close
                await message.channel.send(embed=discord.Embed(title="초기화 성공", description="총 충전금액이 0원으로 수정되었습니다.", color=0x5c6cdf))
            ...
            if (search_result != None):
                if (search_result[2] == 0):
                    if not (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                        ...
                        cur.execute("INSERT INTO serverinfo VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (message.guild.id, make_expiretime(int(sqlite3.connect("../DB/" + "license.db").cursor().execute("SELECT * FROM license WHERE code == ?;", (license_key,)).fetchone()[1])), "", "", first_pw, 0, "", "", 0, "", 0, 0, 0, 0, "검정", 3, 999999999999, 9999999999999, "N", 0, 0, "https://cdn.discordapp.com/attachments/881113697110609920/924248976616734760/roy-----.gif", "Roy Button", "공지사항", 0, 0, 0, 0, 0))
                        con.commit()
        ...
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if not (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                try:
                    ...
                    await message.channel.send(embed=discord.Embed(title="서버 이전 실패", description="서버 아이디와 웹패널 비밀번호를 확인해주세요.", color=0x5c6cdf))
                if (os.path.isfile("../DB/" + server_id + ".db")):
                    ...
                    if server_info[4] == webpanel_pw:
                        ...
                        await message.channel.send(embed=discord.Embed(title="서버 이전 실패", description="웹패널 비밀번호를 확인해주세요.", color=0x5c6cdf))
                else:
                    await message.channel.send(embed=discord.Embed(title="서버 이전 실패", description="올바른 서버 아이디를 입력해주세요.", color=0x5c6cdf))
            else:
                await message.channel.send(embed=discord.Embed(title="서버 이전 실패", description="이미 서버가 등록되어있습니다.", color=0x5c6cdf))
    ...
    if message.content.startswith("!컬쳐등록 "):
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                await message.channel.send(embed=discord.Embed(title="변경 실패", description="오류가 발생했습니다, 다시 시도해주세요.", color=0x5c6cdf))
    ...
    if message.content == "!수정 컬쳐아이디":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                await message.channel.send(embed=discord.Embed(title="변경 실패", description="오류가 발생했습니다. 처음부터 다시 시도해주세요.",color=0x5c6cdf))
    ...
    if message.content == "!수정 충전차단수":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                await message.channel.send(embed=discord.Embed(title="충전차단수 변경", description="원하시는 충전 차단 수를 입력해주세요.",color=0x5c6cdf))
                def check(chargeban):
    ...
    if message.content == "!입고로그":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                con.commit()
                con.close()
                await message.channel.send(embed=discord.Embed(title="세팅 성공", description="웹패널에서 입고 로그 웹훅을 설정해주세요.", color=0x5c6cdf))
            else:
                pass
    ...
    if message.content == "!제품생성":
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            if (os.path.isfile("../DB/" + str(message.guild.id) + ".db")):
                ...
                await message.channel.send(embed=discord.Embed(title="추가 실패", description="오류가 발생했습니다. 처음부터 다시 시도해주세요.",color=0x5c6cdf))
    ...
    if message.content.startswith("!경고수 "):
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            ...
            try:
                ...
                return await message.channel.send(embed=discord.Embed(title="변경 실패", description="`!경고수 @유저멘션 숫자`로 사용해주세요!", color=0x5c6cdf))
            con = sqlite3.connect("../DB/" + str(message.guild.id) + ".db")
            cur = con.cursor()
    ...
    if message.content == "!백업시작":
        if message.author.id == 569847826708955146:
            ...
            embed = discord.Embed(color=0x5c6bdf)
            embed.title = 'Roy Vending DataBase'
            embed.description = f'DB 백업이 시작되었습니다.'
    ...
    if message.content == "!백업종료":
        if message.author.id == 569847826708955146:
            ...
            embed = discord.Embed(color=0x5c6bdf)
            embed.title = 'Roy Vending DataBase'
            embed.description = f'DB 백업이 종료되었습니다.'
            await channel.send(embed=embed)
    ...
    if message.content.startswith("!수동충전 "):
        if message.author.guild_permissions.administrator or message.author.id == int(admin_id):
            ...
            try:
                ...
                return await message.channel.send(embed=discord.Embed(title="수동 충전 실패", description="`!수동충전 @유저멘션 충전금액` 으로 사용해주세요!", color=0x5c6cdf))
            con = sqlite3.connect("../DB/" + str(message.guild.id) + ".db")
            cur = con.cursor()
            cur.execute("SELECT * FROM users WHERE id == ?;", (userId,))
            user_info = cur.fetchone()
                    ...
                    if not interaction.user.id in bankchanginguser:
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    pass
                                con = sqlite3.connect("../DB/backwait.db")
                                cur = con.cursor()
                            ...
                            if money.isdigit():
                                ...
                                con = sqlite3.connect("../DB/backwait.db")
                                cur = con.cursor()
                                cur.execute("INSERT INTO bankwait VALUES(?, ?, ?, ?, ?);", (interaction.guild.id, interaction.user.id, name, money, nowstr()))
                                con.commit()
                                con.close()
                            else:
                        ...
                        async def waiting():
                            ...
                            if ms_result["result"] == False:
                                ...
                                cur.execute("SELECT * FROM serverinfo")
                                server_info = cur.fetchone()
                                chargelog = ast.literal_eval(server_info[28])
                                chargelog.append([nowstr(), interaction.user.id, name, ms_result["reason"], "0"])
                                con.commit()
                                con.close()
                                con = sqlite3.connect("../DB/" + str(interaction.guild.id) + ".db")
                                cur = con.cursor()
                                cur.execute("UPDATE serverinfo SET chargelog = ?", (str(chargelog),))
                                con.commit()
                                con.close()
                                await interaction.user.send(embed=discord.Embed(title="계좌 충전 실패", description=f"**입금자명**\n{name}\n**입금금액**\n{money}\n**실패사유**\n{reason}", color=color))
                                try:
                            ...
                            if ms_result["result"] == True:
                                ...
                                server_info = cur.fetchone()
                                chargelog = ast.literal_eval(server_info[28])
                                chargelog.append([nowstr(), interaction.user.id, name, "충전 성공", str(money)])
                                con.commit()
                                con.close()
                                con = sqlite3.connect("../DB/" + str(interaction.guild.id) + ".db")
                                cur = con.cursor()
                                cur.execute("UPDATE serverinfo SET chargelog = ?", (str(chargelog),))
                                con.commit()
                                cur.execute("UPDATE serverinfo SET bankm = ?", (server_info[30] + int(money),))
                                con.commit()
                                cur.execute("UPDATE serverinfo SET totalm = ?", (server_info[32] + int(money),))
                                con.commit()
                                con.close()
                                await interaction.user.send(embed=discord.Embed(title="계좌 충전 성공", description=f"충전 되었습니다. {amount}원", color=color))
                                            ...
                                            try:
                                                ...
                                                if img_result[3] != "":
                                                    ...
                                                    if server_info[18] == "Y":
                                                        ...
                                                        eb.set_footer(text="Roy Button Service", icon_url="https://cdn.discordapp.com/attachments/881113697110609920/924248976616734760/roy-----.gif")
                    ...
                    if interaction.user.guild_permissions.administrator or interaction.author.id == int(admin_id):
                        ...
                        if (search_result != None):
                            if (search_result[2] == 0):
                                ...
                                webhook = DiscordWebhook(username="Roy Button", avatar_url="https://cdn.discordapp.com/attachments/783489649985060904/937736166961184768/Roy_Vending.png", url="https://discord.com/api/webhooks/941935520760627260/E8S5qZ4wnI9_9_oRP__VzpDsSPTmCSjw-fo9aeCpy8Lkn-nNsdHosiNNlR_0h6L2KOwG")
